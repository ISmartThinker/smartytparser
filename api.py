...
async def fetch_youtube_details(video_id: str):
    try:
        ...
        return {
            ...
            "comments": getValue(player, ["engagementPanels",1,"engagementPanelSectionListRenderer","content","structuredDescriptionContentRenderer","items",1,"videoDescriptionHeaderRenderer","commentCount","simpleText"])
        }
    except:
        return {
            "title": "Unavailable",
...
@app.get("/search")
async def search(q: str, limit: int = 20, mode: str = "videos", sort: str = None, date: str = None, duration: str = None, continuation: str = None):
    start = time.time()
...
async def channel_playlists(channel_id: str, continuation: str = None):
    start = time.time()
...
@app.get("/hashtag")
async def hashtag(tag: str, limit: int = 20, continuation: str = None):
    start = time.time()
    if continuation:
        ...
        if not params: raise HTTPException(404, "Hashtag not found")
